...
HW 21: Game of Life
Last Date Modified: 07/23/2022
Describtion: randomly generate 100 live cells for game of life using turtle
'''
import turtle
import random
import copy
...
lifeturtle.speed(0)
lifeturtle.color(random.random(), random.random(), random.random())
...
def init_lives():
    for i in range(n):
        ...
        for j in range(n):
            if random.randint(0,9) == 0: # 1/9 probability of life which is 100
...
def update_life(): # update life for each cycle
    ...
    draw_all_life()
    screen.update() 
    screen.ontimer(update_life,150) # update life every 0.15 second
